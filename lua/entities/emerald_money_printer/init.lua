...
function ENT:Initialize()
	...
	self:SetColor(Color(0, 112, 60, 255))
...
function ENT:CreateMoneybag()
	...
	local Y = GAMEMODE.Config.emeraldprintamount
	if amount == 0 then
		amount = 25
	end
...
function ENT:Use(activator)
	...
	GAMEMODE:Notify(activator, 1, 4, "You have collected $"..self:GetNWInt("PrintA").." from a Emerald Printer.")
	self:SetNWInt("PrintA",0)
	end
end